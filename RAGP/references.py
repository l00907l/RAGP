...
if __name__ == "__main__":
    ...
    for task_idx, y_data in enumerate(y_files):
        ...
        indices, labels, features, self_labels  = trainer.get_reference(model, val_loader)
        df = pd.DataFrame({
            'group_id': torch.repeat_interleave(
                torch.arange(len(self_labels)), 
                repeats=5
            ).numpy(), 
            'indices': indices.cpu().numpy().flatten(),
            'labels': labels.cpu().numpy().flatten(),
            'self_label': torch.repeat_interleave(
                self_labels, 
                repeats=5
            ).numpy()  # 每个self_label重复5次
        })

        save_dir = os.path.join("./references", config["dataset"])
        os.makedirs(save_dir, exist_ok=True)
        df.to_csv(os.path.join(save_dir, f"task_{task_idx}.csv"), index=False)

        print(f"参考结果已保存到: {save_dir}/task_{task_idx}.csv")